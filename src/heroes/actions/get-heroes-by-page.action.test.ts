...
// peticiones de axios
//utilizamos axios mock adapter
describe('getHeroesByPageAction', () => {

    const heroesApiMock = new AxiosMockAdapter(heroApi);
    // Siempre siempre hacer el beforeEach para que resetee los datos para cada test si no podemos confundirnos
    beforeEach(() => {
        heroesApiMock.reset();
    })
    ...
    test('should call the api with correct paramas', async () => {
        const responseObject = {
            total: 10,
            pages: 1,
            heroes: []
